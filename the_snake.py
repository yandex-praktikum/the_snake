...
class GameObject:
    ...
    def draw(self, surface):
        ...
        error = "Метод draw() должен быть переопределён в дочерних классах."
...
def update_game(snake, apple, bad_food, obstacles):
...
def main():
    ...
    while running:
        ...
        handle_keys(snake)
        bad_food = update_game(snake, apple, bad_food, obstacles)
        screen.fill(BLACK)
        apple.draw(screen)
        if bad_food:
